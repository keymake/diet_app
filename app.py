...
def save_data(df: pd.DataFrame):
    """전체 데이터를 Supabase에 저장 (간단하게 전체 삭제 후 재삽입)."""
    supabase = get_supabase_client()
    ...
    supabase.table("records").insert(data_list).execute()
...
def save_config(config: dict):
    ...
    supabase = get_supabase_client()
    # 모두 지우고 다시 넣는 단순 방식 (설정 항목 적으니까 이게 더 편함)
...
def page_A():
    ...
    new_kcal_bf = st.number_input(
        "아침", min_value=0, step=10, value=int(old_cb.get("아침", 0))
    )
    new_kcal_lu = st.number_input(
        "점심", min_value=0, step=10, value=int(old_cb.get("점심", 0))
    )
    new_kcal_di = st.number_input(
        "저녁", min_value=0, step=10, value=int(old_cb.get("저녁", 0))
    )
    new_kcal_sn = st.number_input(
        "간식", min_value=0, step=10, value=int(old_cb.get("간식", 0))
    )

    new_total_kcal = (
        new_kcal_bf + new_kcal_lu + new_kcal_di + new_kcal_sn
    )
    new_cb_dict = {
        "아침": new_kcal_bf,
        "점심": new_kcal_lu,
    ...
    if st.button("이 날짜 수정 저장"):
        ...
        df.loc[df["date"] == selected_date, "calories_breakdown"] = (
            new_cb_dict
        )
        ...
        if prev_pos >= 0:
            prev_weight_val = float(
                before_df.iloc[prev_pos]["weight"]
            )
        else:
